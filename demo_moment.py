...
The original ladder code is in demo_ladder.py, which is imported for
waveform comparing.
...
def momentMatching(N=10, qParam=1, endTime=300, deltaTime=0.1):
    ...
    + qPararm -> using how many approximate orders to simulate exact wave
...
if __name__ == '__main__':
    ...
    ## from ladderWave().
    ## Here, for clarity and full convergency, we set time=20000, and try
    ## N=10, qParam=1
    ## N=100, qParam=2
    ...
    t1 = time.clock()
    (v, t) = lad.ladderWave(N=order, endTime=20000)
    lad.plot_wave(t, v, 'r--', name='N=' + str(order) + ' original')

    t2 = time.clock()

    data = momentMatching(N=order, qParam=reduction_order, endTime=20000)
